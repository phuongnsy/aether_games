...
uniform vec4 u_hero;  // xy=light screen uv, z=screen radius, w=softness steps
...
void main()
{
	...
	vec2 toLight = u_hero.xy - suv;
	float shadow = 0.0;
	float steps = 24.0;
	// March from the fragment toward the light but STOP SHORT of it (x0.85): an
	// occluder right AT the light (e.g. the caster pushed against a wall) must not
	// shadow its own pool. Samples run from just off the fragment to 0.85 of the way.
	for (float i = 1.0; i <= 24.0; i += 1.0) {
		vec2 p = suv + toLight * (i / steps * 0.85);
		shadow += step(0.5, texture2D(s_texColor, p).a);
	}
	float lit = 1.0 - clamp(shadow / u_hero.w, 0.0, 1.0);
	gl_FragColor = vec4(v_color0.rgb * atten * ndl * lit, 1.0);
}
